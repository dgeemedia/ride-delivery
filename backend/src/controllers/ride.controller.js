...
// Helper: emit ride status to customer + ride room
...
exports.requestRide = async (req, res) => {
  ...
  broadcastToDrivers(getIO(req), 'ride:new_request', {
    ...
    customer: { firstName: ride.customer.firstName, lastName: ride.customer.lastName, profileImage: ride.customer.profileImage }
  });
...
exports.acceptRide = async (req, res) => {
  ...
  await notificationService.notify({
    ...
    type: notificationService.TYPES?.RIDE_ACCEPTED || 'RIDE_ACCEPTED',
    data: { rideId: id, driverName: `${req.user.firstName} ${req.user.lastName}`, vehicleType: driverProfile.vehicleType, vehiclePlate: driverProfile.vehiclePlate, vehicleColor: driverProfile.vehicleColor }
  });
...
exports.arrivedAtPickup = async (req, res) => {
  ...
  await notificationService.notify({
    ...
    type: notificationService.TYPES?.RIDE_ARRIVED || 'RIDE_ARRIVED',
    data: { rideId: id }
  });
...
exports.startRide = async (req, res) => {
  ...
  await notificationService.notify({
    ...
    type: notificationService.TYPES?.RIDE_STARTED || 'RIDE_STARTED',
    data: { rideId: id }
  });
...
exports.completeRide = async (req, res) => {
  ...
  await notificationService.notify({
    ...
    type: notificationService.TYPES?.RIDE_COMPLETED || 'RIDE_COMPLETED',
...
exports.cancelRide = async (req, res) => {
  ...
  // ✅ Emit cancellation to both parties
  const io = getIO(req);
  if (io) {
  ...
  if (notifyUserId) {
    await notificationService.notify({
      ...
      type: notificationService.TYPES?.RIDE_CANCELLED || 'RIDE_CANCELLED',
...
exports.getNearbyDrivers = async (req, res) => {
  ...
  const formatted = nearby.map(d => ({
    driverId: d.user.id, profileId: d.id,
    firstName: d.user.firstName, lastName: d.user.lastName, profileImage: d.user.profileImage,
    vehicleType: d.vehicleType, vehicleMake: d.vehicleMake, vehicleModel: d.vehicleModel,
    vehicleColor: d.vehicleColor, vehiclePlate: d.vehiclePlate, vehicleYear: d.vehicleYear,
    rating: parseFloat((d.rating || 0).toFixed(1)),
    totalRides: d.totalRides, distanceKm: d.distanceKm,
