...
import {
  ...
  StatusBar, ActivityIndicator, Animated, Dimensions, Platform,
} from 'react-native';
...
export default function LegalScreen({ navigation, route }) {
  ...
  // KEY: measured header height for accurate SCROLL_H — same pattern as
  // ProfileScreen / SupportScreen.
  const [headerH, setHeaderH] = useState(56);

  // KEY: bounded scroll area height.
  ...
  useEffect(() => { fetchContent(); }, [contentKey]);
  ...
  return (
    <View style={[st.root, { backgroundColor: theme.background }]}>
      ...
      {/* FIX: pointerEvents="none" so the glow never absorbs taps. */}
      <View style={[st.glow, { backgroundColor: theme.accent }]} pointerEvents="none" />

      {/* ── Sticky header — replaces the old absolute back button + titleBar.
           Measured via onLayout so SCROLL_H is always pixel-perfect across
           all devices and font-scale settings.                              */}
      <View
        style={[st.header, {
      ...
      >
        <TouchableOpacity
          style={[st.backBtn, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}
        ...
        <Text style={[st.headerTitle, { color: theme.foreground }]}>{title}</Text>
        {/* Spacer mirrors backBtn width to keep title centred */}
        <View style={st.headerSpacer} />
      </View>

      {/* ── Content — bounded by SCROLL_H so it never overflows ─────────── */}
      {loading ? (
        ...
        // KEY: bounded pixel-height container gives ScrollView a concrete parent
        // boundary — same pattern as ProfileScreen / SupportScreen.
...
const st = StyleSheet.create({
  ...
  // ── Sticky header (replaces absolute back button + separate titleBar) ──────
  header: {
  ...
  headerTitle:  { flex: 1, textAlign: 'center', fontSize: 17, fontWeight: '700', letterSpacing: -0.2 },
  headerSpacer: { width: 38 },

  // ── Scroll content ────────────────────────────────────────────────────────
  scroll:  { paddingHorizontal: 24, paddingTop: 24 },
  centre:  { justifyContent: 'center', alignItems: 'center', paddingHorizontal: 32 },
