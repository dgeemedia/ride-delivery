...
//
// Shared transaction history screen used by Customer, Driver, and Partner.
// Features:
//   • Custom date-range filter (from / to date pickers)
//   • Transaction type filter chips
//   • Full transaction list with running balance
//   • Export to PDF (generates + shares via expo-sharing)
//   • Send PDF to registered email via API
//
// Props (via navigation params):
//   • userType: 'customer' | 'driver' | 'partner'  (optional — auto-detected from role)
//   • initialFrom / initialTo: Date strings for default range
//
// API dependencies (walletAPI):
//   • getTransactions({ from, to, type, page, limit })
//   • emailTransactionHistory({ from, to, type, email })
//
// Native deps needed:
//   expo-sharing, expo-print, @react-native-community/datetimepicker
...
let Sharing, Print, DateTimePicker;
...
// ────────────────────────────────────────────────────────────────────────────────
// DatePickerModal — simple cross-platform date picker wrapper
// ────────────────────────────────────────────────────────────────────────────────
        ...
        <View style={dp.overlay}>
          <View style={[dp.card, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
            ...
            <input
              ...
              style={{ fontSize: 16, padding: 8, borderRadius: 8, border: `1px solid ${theme.border}`, color: theme.foreground, background: theme.background, marginBottom: 16 }}
            />
            <View style={dp.btns}>
              <TouchableOpacity style={[dp.btn, { borderColor: theme.border }]} onPress={onClose}>
                <Text style={[dp.btnTxt, { color: theme.hint }]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[dp.btnAccent, { backgroundColor: accent }]} onPress={() => { onChange(local); onClose(); }}>
                <Text style={[dp.btnTxt, { color: '#fff' }]}>Set Date</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
export default function TransactionHistoryScreen({ route, navigation }) {
  ...
  const handleExportPdf = async () => {
    if (!Print) {
      Alert.alert('Not supported', 'PDF export requires expo-print.');
      return;
    }
    setExporting(true);
    try {
  ...
  const handleEmailPdf = async () => {
    ...
    try {
      ...
      Alert.alert('Failed', err?.message ?? 'Could not send email. Try again.');
    } finally {
      setEmailing(false);
    }
  };
            ...
            <View style={[s.emailPanel, { backgroundColor: theme.backgroundAlt, borderColor: accent + '40' }]}>
              ...
              <View style={s.emailActions}>
                ...
                >
                  {emailing
                    ? <ActivityIndicator size="small" color="#fff" />
                    : <><Ionicons name="send-outline" size={13} color="#fff" /><Text style={s.emailSendTxt}>Send PDF</Text></>
                  }
                </TouchableOpacity>
              </View>
            </View>
          ...
          {!loading && transactions.length > 0 && (
            <View style={s.exportRow}>
              ...
              >
                {exporting
                  ? <ActivityIndicator size="small" color="#fff" />
                  : <><Ionicons name="document-text-outline" size={15} color="#fff" /><Text style={[s.exportBtnTxt, { color: '#fff' }]}>Download PDF</Text></>
                }
              </TouchableOpacity>
            </View>
          )}
...
const s = StyleSheet.create({
  ...
  emailSendTxt:    { fontSize: 13, fontWeight: '800', color: '#fff' },
