...
import {
  ...
  Alert, StatusBar, Animated, Platform, ActivityIndicator,
} from 'react-native';
...
const NotifRow = ({ item, theme, onPress }) => {
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        nr.row,
        { backgroundColor: item.isRead ? 'transparent' : theme.accent + '09', borderBottomColor: theme.border },
      ]}
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={[nr.message, { color: theme.muted }]} numberOfLines={2}>{item.message}</Text>
...
export default function NotificationsScreen({ navigation }) {
  ...
  const renderFooter = () => (
    hasMore && !loading ? (
      <TouchableOpacity style={[s.loadMore, { borderColor: theme.border }]} onPress={() => loadNotifications(page + 1)}>
        <Text style={[s.loadMoreTxt, { color: theme.accent }]}>{fetching ? 'Loading…' : 'Load more'}</Text>
      </TouchableOpacity>
    ) : null
  );
    ...
    <View style={[s.root, { backgroundColor: theme.background }]}>
      ...
      <View style={[s.header, { borderBottomColor: theme.border }]}>
        ...
        <View>
          ...
          {unread > 0 && <Text style={[s.headerSub, { color: theme.accent }]}>{unread} unread</Text>}
        </View>
        <View style={s.headerActions}>
          {unread > 0 && (
            <TouchableOpacity onPress={handleMarkAllRead} style={[s.headerBtn, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
              <Ionicons name="checkmark-done-outline" size={16} color={theme.accent} />
            </TouchableOpacity>
          )}
          {notifs.length > 0 && (
            <TouchableOpacity onPress={handleClearAll} style={[s.headerBtn, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
      ...
      <Animated.View style={[{ flex: 1 }, { opacity: fadeA }]}>
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => <NotifRow item={item} theme={theme} onPress={handleMarkRead} />}
