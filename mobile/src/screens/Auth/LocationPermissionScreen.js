...
export default function LocationPermissionScreen({ onRequest, onSkip }) {
  ...
  const handleEnable = async () => {
    ...
    // Pass the result back to the gate
    onRequest(status === 'granted');
  };
  ...
  return (
    <View style={[s.root, { backgroundColor: theme.background }]}>
      ...
      <View style={s.content}>
        ...
        {/* Buttons */}
        <TouchableOpacity
          style={[s.primaryBtn, { overflow: 'hidden' }]}
        ...
        >
          ...
          <Text style={[s.primaryTxt, { color: '#000' }]}>Enable Location</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Text style={[s.skipTxt, { color: theme.hint }]}>Maybe Later</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
