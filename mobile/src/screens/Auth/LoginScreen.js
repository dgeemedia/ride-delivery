...
const FloatInput = ({ label, iconName, value, onChangeText, keyboardType, secureTextEntry }) => {
  ...
  return (
    <Animated.View style={[s.inputBox, { backgroundColor: G.card(mode), borderColor: borderC }]}>
      ...
      />
      <View style={{ flex: 1 }}>
        <Animated.Text style={[s.floatLabel, { top, fontSize: fSize, color: lColor }]}>{label}</Animated.Text>
        <TextInput
...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    // 2FA required → push to OTP screen
    if (res.requiresOtp) {
...
const s = StyleSheet.create({
  ...
  header: { marginTop: Platform.OS === 'ios' ? 72 : 56, marginBottom: 44 },
  ...
  bioTxt: { fontSize: 15, fontWeight: '700' },
