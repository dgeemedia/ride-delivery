...
//
// Shows driver earnings, wallet balance, and lets them withdraw to a bank.
// Calls:
//   driverAPI.getEarnings(?period=today|week|month|all)
//   walletAPI.getWallet()
//   walletAPI.getTransactions()
//   driverAPI.requestPayout({ amount, accountNumber, bankCode, accountName })

import React, { useState, useEffect, useRef, useCallback } from 'react';
import {
  View, Text, StyleSheet, TouchableOpacity, ScrollView,
  TextInput, Modal, ActivityIndicator, Alert, Animated,
  Dimensions, StatusBar, Platform, FlatList,
} from 'react-native';
...
// ─────────────────────────────────────────────────────────────────────────────
// Nigerian banks list (top 20 most used)
// ─────────────────────────────────────────────────────────────────────────────
const NG_BANKS = [
...
// ─────────────────────────────────────────────────────────────────────────────
const TX_CONFIG = {
  CREDIT:     { color: '#5DAA72', icon: 'arrow-down-circle-outline', prefix: '+' },
  DEBIT:      { color: '#E05555', icon: 'arrow-up-circle-outline',   prefix: '-' },
  WITHDRAWAL: { color: DA,        icon: 'cash-outline',               prefix: '-' },
  REFUND:     { color: '#A78BFA', icon: 'refresh-circle-outline',    prefix: '+' },
};

// ─────────────────────────────────────────────────────────────────────────────
// StatStrip — the big three numbers at the top
// ─────────────────────────────────────────────────────────────────────────────
const StatStrip = ({ earnings, walletBalance, theme }) => (
  <View style={[ss.card, { backgroundColor: theme.backgroundAlt, borderColor: DA + '30' }]}>
    <View style={ss.item}>
      ...
      <Text style={[ss.val, { color: '#5DAA72' }]}>
...
// ─────────────────────────────────────────────────────────────────────────────
// EarningRow — single completed ride in the history list
// ─────────────────────────────────────────────────────────────────────────────
  ...
  <View style={[er.row, !last && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
    ...
    <View style={{ flex: 1, minWidth: 0 }}>
      ...
      <Text style={[er.date, { color: theme.hint }]}>
        {item.completedAt ? new Date(item.completedAt).toLocaleDateString('en-NG', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' }) : '—'}
      </Text>
    </View>
...
// ─────────────────────────────────────────────────────────────────────────────
// TxRow — wallet transaction history row
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <View style={[tr.row, !last && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        ...
        <Text style={[tr.date, { color: theme.hint }]}>
          {item.createdAt ? new Date(item.createdAt).toLocaleDateString('en-NG', { day:'numeric', month:'short', year:'numeric' }) : '—'}
        </Text>
      </View>
...
const WithdrawModal = ({ visible, walletBalance, onClose, onSuccess, theme }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (amt > walletBalance)  return Alert.alert('Insufficient balance', `Your wallet has ₦${walletBalance?.toFixed(2)}.`);
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={handleClose}>
      <View style={[wm.root, { backgroundColor: theme.background }]}>
        ...
        <View style={[wm.header, { borderBottomColor: theme.border }]}>
          ...
          <TouchableOpacity onPress={handleClose} style={[wm.closeBtn, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
        ...
        <ScrollView contentContainerStyle={wm.scroll} keyboardShouldPersistTaps="handled">
            ...
            <View style={[wm.bankDropdown, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
              ...
              <ScrollView style={{ maxHeight: 200 }} nestedScrollEnabled>
                {filteredBanks.map(b => (
                  <TouchableOpacity
                    key={b.code}
                    style={[wm.bankItem, { borderBottomColor: theme.border }, selectedBank?.code === b.code && { backgroundColor: DA + '14' }]}
                    onPress={() => { setSelectedBank(b); setShowBanks(false); setBankSearch(''); }}
                  >
                    <Text style={[wm.bankItemTxt, { color: selectedBank?.code === b.code ? DA : theme.foreground }]}>{b.name}</Text>
          ...
          <View style={[wm.inputWrap, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
            ...
            <TextInput
              ...
              maxLength={10}
            />
            <Text style={[wm.charCount, { color: accountNo.length === 10 ? DA : theme.hint }]}>{accountNo.length}/10</Text>
          </View>
          ...
          <View style={[wm.noteCard, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
            ...
            <Text style={[wm.noteTxt, { color: theme.hint }]}>
              Funds arrive within 1–2 business days. A 20% platform fee has already been deducted from your earnings.
            </Text>
          </View>
          ...
          >
            {submitting ? (
              ...
              <>
                ...
                <Text style={wm.submitTxt}>Withdraw ₦{amount ? Number(amount).toLocaleString('en-NG') : '—'}</Text>
              </>
            )}
          </TouchableOpacity>
        </ScrollView>
...
export default function EarningsScreen({ navigation }) {
  ...
  const [activeTab,     setActiveTab]    = useState('rides');   // 'rides' | 'transactions'
  ...
  const handleWithdrawSuccess = (amt) => {
    ...
    Alert.alert('Withdrawal Initiated 🏦', `₦${Number(amt).toLocaleString('en-NG')} will arrive in your bank within 1–2 business days.`);
    fetchAll(period);
  };
  ...
  return (
    <View style={[s.root, { backgroundColor: theme.background }]}>
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity
          style={[s.withdrawBtn, { backgroundColor: DA }]}
        ...
        >
          ...
          <View style={[s.breakdownCard, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
            ...
            {[
              ...
              ['Platform Fee (20%)', earnings?.platformFee, '#E05555'],
              ['Net Earnings',    earnings?.netEarnings,  '#5DAA72'],
          ...
          <View style={s.periodRow}>
            {PERIODS.map(p => {
              ...
              return (
                <TouchableOpacity
                  key={p.key}
                  style={[s.periodBtn, active && { backgroundColor: DA }, !active && { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}
          ...
          <View style={[s.tabRow, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]}>
            {[['rides', 'Ride History'], ['transactions', 'Wallet Txns']].map(([key, lbl]) => (
              <TouchableOpacity
                key={key}
                style={[s.tabBtn, activeTab === key && { backgroundColor: DA + '20', borderBottomColor: DA, borderBottomWidth: 2 }]}
...
const s = StyleSheet.create({
  ...
  // Header
  header:     { flexDirection: 'row', alignItems: 'flex-end', gap: 12, paddingHorizontal: 24, paddingTop: Platform.OS === 'ios' ? 60 : 48, paddingBottom: 16 },
  backBtn:    { width: 40, height: 40, borderRadius: 12, borderWidth: 1, justifyContent: 'center', alignItems: 'center', marginBottom: 2 },
  eyebrow:    { fontSize: 10, fontWeight: '800', letterSpacing: 3, marginBottom: 3 },
  title:      { fontSize: 26, fontWeight: '900', letterSpacing: -0.3 },
  withdrawBtn:{ flexDirection: 'row', alignItems: 'center', gap: 5, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10, marginBottom: 2 },
  withdrawBtnTxt: { fontSize: 13, fontWeight: '800', color: '#080C18' },
