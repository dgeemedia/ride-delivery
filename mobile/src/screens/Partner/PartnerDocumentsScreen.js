...
const DOC_TYPES = [
  {
    ...
    icon: 'card-outline',
    required: true,
    uploadFn: uploadAPI.uploadPartnerId,
  },
  {
    key: 'vehicleImageUrl',
    label: 'Vehicle Photo',
    icon: 'car-outline',
    required: false,
    uploadFn: uploadAPI.uploadPartnerVehicle,
  },
];
...
export default function PartnerDocumentsScreen({ navigation }) {
  ...
  const pickAndUpload = async (docType) => {
    ...
    try {
      ...
      await partnerAPI.updateProfile(updatePayload);
    ...
    <View style={[s.root, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <Text style={[s.subtitle, { color: theme.hint }]}>
          Upload your ID and vehicle photo for verification.
        </Text>
